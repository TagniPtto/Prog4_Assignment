...
namespace ImGui {
// [0..1] -> [0..1]
static float rescale(float t, float min, float max, PlotConfig::Scale::Type keyType) {
    switch (keyType) {
...
// [0..1] -> [0..1]
static float rescale_inv(float t, float min, float max, PlotConfig::Scale::Type keyType) {
    switch (keyType) {
    case PlotConfig::Scale::Linear:
        return t;
    case PlotConfig::Scale::Log10:
...
static int cursor_to_idx(const ImVec2& pos, const ImRect& bb, const PlotConfig& conf, float x_min, float x_max) {
    ...
    const int v_idx = (int)(rescale_inv(t, x_min, x_max, conf.scale.keyType) * (conf.values.count - 1));
    ...
    if (conf.values.count > 0) {
        ...
        for (int i = 0; i < ys_count; ++i) {
            ...
            for (int n = 0; n < res_w; n++)
            {
                ...
                const ImVec2 tp1 = ImVec2(
                    rescale(t1, x_min, x_max, conf.scale.keyType),
        ...
        if (conf.v_lines.show) {
            for (size_t i = 0; i < conf.v_lines.count; ++i) {
                ...
                const float t1 = rescale(idx * t_step, x_min, x_max, conf.scale.keyType);
